    ...
    >
      ...
      {hover === 1 && (
        <div className="absolute top-[10%] right-60 w-80   text-justify bg-white p-5  rounded-2xl border-2">
          ...
          operational consistency, compliance, and customer satisfaction.
        </div>
      )}
      {hover === 2 && (
        <div className="absolute top-[35%] right-14 w-80 text-justify p-5 bg-white rounded-2xl border-2 ">
          "Streamline Success: Optimize Processes, Maximize Impact" Transform
          workflows and eliminate inefficiencies with tailored process
          improvement consulting for sustainable operational excellence.
        </div>
      )}
      {hover === 3 && (
        <div className="absolute top-[60%] right-44 w-80 text-justify p-5 bg-white rounded-2xl border-2 ">
          "Insights That Empower: Measure, Predict, Transform" Leverage
          data-driven KPI design and AI-powered dashboards to gain actionable
          insights and predictive analytics for smarter decisions.
        </div>
      )}
      {hover === 4 && (
        <div className="absolute top-[60%] left-28 w-80 text-justify p-5 bg-white rounded-2xl border-2 ">
          "Continuously Innovate: Build a Future-Ready Organization" Embed a
          culture of continuous improvement with robust frameworks, innovation
          councils, and benefit measurement strategies.
        </div>
      )}
      {hover === 5 && (
        <div className="absolute top-[25%] left-20 w-80 text-justify p-5 bg-white rounded-2xl border-2 ">
          "AI at Your Fingertips: Tailored Training, Seamless Deployment" Equip
